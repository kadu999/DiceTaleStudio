...
| 编辑器 `runtime_start` | **开闸**：此后 `/client` 可连接 |
| 编辑器 `runtime_stop` / 编辑器 WS 断开 | **关闸**：踢掉已连前端（close **4003**）、清场景缓存 |

「运行态」按**编辑器会话**记账（任一编辑器声明了运行态即开闸；全部退出才关闸）。
编辑器刷新页面会踢一次前端——**刻意如此**：运行态跟着编辑器活着，不留没人管的「已连接」。
前端本来就在一直重试（默认 3s），编辑器再次点运行时它就自己连回来了。
