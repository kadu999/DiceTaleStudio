...
- `softness`：软边带比例（0 = 硬边、1 = 全程衰减）。编辑器固定 `1`（战争雾）；视频混合用 `0.5`
  ——`1` 的圆刷没有平顶核，`min` 幂等下擦不到 0，「擦完的区域」会永远糊着一层 A
  （见 `apps/editor/src/services/mask-math.ts` 的 `VIDEO_BLEND_MASK_SOFTNESS`）；
