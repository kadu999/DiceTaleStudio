 ...
 *
 * v29（2026-09-27）：**视频混合的两路从「列表 + 选中」收成单个素材**（`VideoBlendChannelDoc` =
 * `{ kind: "image" | "video", id? }`）：一个混合层只显示一路，列表给不了额外能力，
 * 而 `kind` 让「静图盖住视频」这类用法也能做。由 `migrateVideoBlendChannels` 搬一次
 * （取选中那条；那时只可能是视频）。协议侧同步升到 v19。
 *
 * v30（2026-09-27）：**新增「放大镜」动作对象**（`kind: "Magnifier"` + 必需组件 `Magnifier`）。
 * 它自己什么都不渲染，声明的是「告诉前端弹一扇窗、窗里放哪张图」（图片列表 + 当前展示的那一张）。
 * **没有迁移函数**：老文件里既没有这个 kind、也没有这个组件，纯加法；格式 +1 是为了
 * 老编辑器撞上 `kind: z.enum(OBJECT_KINDS)` 时拿到一句**明确的**「请升级编辑器」
 * （与 `Fog`（v27）那条老规矩一致）。协议侧同步升到 v21（新组件 + 开 / 关两条命令）。
 */
export const DOCUMENT_FORMAT_VERSION = 30;
 ...
 * 只在画布上画一枚固定的内置放大镜徽标（不给换贴图，与播放声音 / 传送阵同一条规矩）。
 *
 * 分工（界面上两处，别混）：
 * - **图片列表**（`images`）在属性面板的「放大镜」组里加 / 移出（每一项都从精灵素材里挑）；
 * - **当前展示哪一张**（`picked`）在面板的小方块上点，或者在那扇窗的下排小图上点
 *   ——前端窗里放的就是它。
 *
 * 每一项直接复用 {@link ImageRef}：它就是「一张图的引用（可以取图集里的一格）」那个形状，
 * 所以「支持精灵」不需要多一个字段；存盘时 `id` 按同一套换算成 GUID
 * （`scene-asset-refs.ts`）、推送时把 `spriteGrid` 解析进载荷（`sprites.ts`）。
 *
 * `picked` 是**下标**（不是 id、也不是「id + 格子」拼的键）：这是一份**有序的**列表，
 * 「当前展示第几张」本来就是位置；而同一张图的两个不同格子是两条，id 当键会撞车。
 */
export interface MagnifierDataDoc {
  /**
   * 加进来的图片，顺序 = 加进来的先后（就是那扇窗下面那一排的次序）；空数组 = 还没加图。
   *
   * 加 / 移出都在属性面板的「放大镜」组里做。一条都还没加时窗口中间写「还没选图」，
   * 「在画面上打开」点不了（前端那边会明确拒掉）。
   */
  readonly images: ImageRef[];
  /**
   * `images` 里**当前展示的那一张**（下标，0 起）；缺省 = 还没选。
   *
   * 换一张是**场景数据**（重开项目还在、撤销能回退），不是界面偏好——前端窗里放的就是它，
   * 运行态下文档一改，整份 `scene_push` 会把新值带到前端（**没有**「换图」这条命令）。
   * 越界（手写文件 / 列表被改短）不算解析错误：`validateScene` 报 warning 并按「还没选」处理。
   */
