...
import { CONCRETE_KINDS, OBJECT_KINDS, kindIsA, type ObjectKind } from "@dts/document";
import {
  KIND_LABELS,
  OBJECT_CATEGORIES,
  categoryOfKind,
 ...
 * 层级（谁是基类、谁是子类型）在 `@dts/document` 的 `kinds.ts`，不在这张表里。
 */
describe("对象类型表（object-kinds.ts）", () => {
  ...
  it("抽象基类只作归类项：不可创建、也不在弹框的候选里", () => {
    const base = OBJECT_CATEGORIES.flatMap((category) => category.objects).find(
      (object) => object.kind === "SceneObject",
    );
    expect(base).toBeDefined();
    expect(base?.creatable).toBe(false);
    expect(base?.label).toBe("场景对象");
    // 它仍然有种类归属（手写文件里出现这个值时，面板不会把它漏掉）
    expect(categoryOfKind("SceneObject")?.id).toBe("entity");

    for (const category of OBJECT_CATEGORIES) {
      expect(
        creatableObjects(category).some((object) => object.kind === "SceneObject"),
        category.id,
      ).toBe(false);
    }
  });
  ...
  it("实体下可创建的就是三个具体类型，顺序为 网格地图 / 精灵 / 贴图", () => {
    ...
    // 精灵是基类的子类型（层级的事实来自 `kinds.ts`，这里只确认表用的是那个值）
    expect(kindIsA("Sprite" as ObjectKind, "SceneObject")).toBe(true);
  });
});
