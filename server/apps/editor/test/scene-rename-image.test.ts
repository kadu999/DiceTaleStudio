 ...
 * 除非把网格地图的贴图引用一起改指到新名字。
 *
 * 这条用例是**回归护栏**：贴图搬进组件之后（v19 搬进 `GridMap`、v28 再搬进 `ImageLayer`），
 * 这段代码曾经还在往对象上写一个扁平的 `map` 字段——schema 会把它当未知键丢掉，于是
 * 「改名成功、贴图丢失」，而且类型检查抓不到。
 */
...
function file(objects: readonly GameObjectDoc[]): SceneFileDoc {
  return { formatVersion: 28, objects: [...objects] };
}

describe("重命名场景：同名贴图跟着改指", () => {
  it("网格地图的贴图指向旧场景名时，改指到新场景名（住在图片层组件里）", () => {
    ...
    expect(imageOf(object)?.id).toBe("project:测试/Assets/images/新名.png");
    // 关键：**没有留下一个扁平的 map 字段**（它会被 schema 丢掉，等于贴图丢了）
    expect((object as unknown as Record<string, unknown>).map).toBeUndefined();
    // 图片层实例还是原来那一个（id 不变、只有 data 换了），显示顺序跟着保留
    expect(componentOf(object, DEFAULT_SLOT_COMPONENT.image)?.id).toBe("map-1__ImageLayer");
    expect(imageLayerDataOf(object)?.sortingOrder).toBe(-10);
    // 网格数据一个字节都没动
  ...
  it("网格地图用的是别的贴图（手工指定的）时不动它", () => {
    const handPicked = withFeature(mapFile(), DEFAULT_SLOT_COMPONENT.image, {
      ...imageLayerDataOf(mapFile())!,
      id: "project:测试/Assets/images/Bridge.png",
    });
