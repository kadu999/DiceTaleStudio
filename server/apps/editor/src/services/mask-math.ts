 ...
 *
 * **两端的对应实现**（改一边就要改另一边）：
 * - 本文件（编辑器预览：Mask 窗口的 canvas + `ImageData`）；
 * - `DiceTale/client/Assets/DiceTale/Scripts/Presentation/FogOfWar.cs`（运行时的雾层：
 *   CPU 同一套公式，遮罩尺寸也用同一个 `previewMaskSizeFor` 口径）。
 *
 * **出处（参考实现 `backend_diceTale`，两处逐字对齐）**：
 ...
 *    → `0.05`；前端收到后再乘**它自己**的遮罩宽。两端的遮罩是**同一张尺寸**，
 *    所以两边擦出的是同一片纹素；
 * 2. **补点步长 = 半径的一半**（纹理像素，且不小于 1）——两边同式，快拖时打的点也一样密；
 * 3. **y 方向**：GM 画布是左上原点、y 向下；前端把它翻转成纹理的自下而上（`(1-y) × 高`）。
 *    本编辑器的遮罩行序也是 y 向下，所以翻转只发生在前端那一侧。
 *
 * 为什么是像素而不是格子：遮罩是一张**纹理**（战争雾的雾层 / `MaskImage` 的 RenderTexture），
 * GM 在图上擦的是软边圆刷；格子掩码是**另一件事**（雾区绑定、`.bytes` 里那套）。
 *
 * **有意与前端不同的两处**（写在这里，免得日后被当成 bug）：
 * 1. **颜色**：前端雾层是统一的雾色（默认黑不透明），这里只把「已指定雾区的格子」按区域配色画上
 *    （见 `fillFogMaskPixels`）——编辑器要的是「雾盖在哪、哪块是哪区」；
 * 2. **不落盘**：这里擦了不写文档（关掉重开回到未探索），但**运行态下会下发**——
 *    轨迹经 `erase_mask` 发给前端，前端擦在自己那份雾层上（那份才是跑团现场的真状态）。
...
/**
 * **视频混合**擦遮罩用的软边比例：`0.5` —— 与雾的 `1` **有意不同**，因为它必须有**实心核**。
 *
 * `softness = 1` 时 `core = radius × (1 - 1) = 0`：整个圆刷从头到尾线性衰减、没有平顶核，而擦除是
 * `min` 幂等（同一处擦 N 次 = 擦 1 次）。于是**擦得再密也到不了 0**——只有笔尖正中心那一个纹素是 0：
 * 来回涂满一片区域后，遮罩 alpha 最低仍有 ~3、均值 ~44/255（按本文件这套像素运算复刻实测）。
 * 对雾无所谓（雾本来就是半透明的），对视频混合却是 bug：混合 shader 是 `lerp(B, A, mask.a)`，
 * alpha 44 → 那块地方永远糊着一层 ~17% 的 A，「擦完的区域」看着还是**两条视频混在一起**，
 * 而不是「擦开露出干净的 B」。
 *
 * `0.5` = 半径的一半是实心核（真的擦到 0）、外圈一半软边（边缘不生硬）。落点间距是半径的一半
 * （`strokeStampCenters`），所以扫过去的那条带子**整条都是 0**；同一片区域来回涂就整片为 0。
 */
export const VIDEO_BLEND_MASK_SOFTNESS = 0.5;
 ...
 * 宽度钉在 960 是因为**前端的遮罩默认就是这么大**——它的笔刷固定 48 纹理像素，于是归一化半径
 * 正好是 `48/960 = 0.05`（真正跨端传的那个数）。C# 侧
 * （`Presentation/FogOfWar.cs` 的 `MaskSizeFor`）是这同一个函数的移植，两边尺寸必须一致。
 */
 ...
 * 前端那边统一是雾色（`FogOfWar` 的 `fogColor`，默认黑不透明），**形状一致、颜色不同**——
 ...
 * （按区域配色重画）。
 *
 * 与前端（`FogOfWar.RevealRegion`）**同一口径**：按**单个区域位**——含该位的格子整块变，
 * 所以一格同时属于两个雾区时会被**整块**揭示/盖回（像素遮罩一格只有一块纹素，
 * 分不出「只擦掉其中一位的那一半」；这一点也写在 README 里）。
 * **盖回会连带盖掉这一区里手动擦掉的部分**（按顺序重放时它排在那些笔画后面），两边一致。
 *
