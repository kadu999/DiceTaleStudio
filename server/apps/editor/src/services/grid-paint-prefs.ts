 ...
 * 以及画布上**网格线**、**网格标注**两个总开关（都是所有地图一起生效的纯显示项）。它们**不进文档**——
 * 对齐 Unity：那几项存在 `GridMapEditorWindow` 的序列化字段里（每个编辑窗口自己记着），
 * 而地图数据（格子掩码）才进 `.bytes` / 场景文件。所以换个项目、甚至重开浏览器，显示方式与配色都还在。
 *
 * **战争雾不在这里**：「开不开战争雾」是**这张地图自己的数据**（`map.fog.enabled`）——
 * 它决定前端生不生成那一层雾，所以必须进文档、随场景下发，不能记在浏览器本地。
 *
...
/** 默认偏好：区域1 画笔、1 号画笔、全部显示、Unity 的默认配色；网格线与标注都画。 */
