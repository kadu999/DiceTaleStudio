...
**关键：对象挂在场景上，不挂在地图上**——所以没有地图也能在场景里建对象；
...
| 页签 | 作用 |
|---|---|
| 项目资源 | 当前跑团文件夹的浏览器（新建文件夹 / 导入资源 / 删除资源） |
| 场景 | 场景列表：新建、切换、重命名、删除 |
| 对象容器 | 当前场景里的对象（新建对象 / 添加地图对象） |
...
└─ campaigns/                         ★ 所有跑团工程
   └─ 我的跑团/                        ★ 一个跑团一个唯一文件夹
      ...
      ├─ config/                       该跑团自己的配置
      ├─ maps/                         地图导出数据：<场景>.bytes（Unity 位精确兼容）
      ├─ images/maps/                  地图贴图：<场景>.png（与场景同名）
      ├─ audio/
      ├─ video/
...
场景数据（含网格与对象）存在**工程文件**里，改了就存；`maps/`、`images/` 等目录放导出物与素材。
