...
**没有「保存场景」按钮**，也不用有：场景内容在编辑器里改不了，新建 / 改名 / 删除都是即时落盘的
文件操作，不存在「改了没存」的中间态。等组件编辑、网格绘制落地后，才需要这个按钮。
...
| 场景对象 | 当前场景里的对象（**只读**列出；地图只是其中之一） |

**编辑器对场景内容只读**：除了上面三个菜单动作，它不写场景文件。
对象的「新建对象 / 添加地图」已移除——新建出来的是空壳（无组件、无动作、不可编辑），
添加地图也不改变画面（渲染器暂不加载贴图），两个按钮只能造出「列表里多一行」的假象。
当前场景取「按名字排序的第一个」，底栏显示场景总数与当前场景名。
...
所以资源面板目前是**只读**的（没有新建 / 导入 / 删除入口）。唯一的例外是场景的三个菜单动作
（新建 / 重命名 / 删除），它们只做文件级操作，不碰场景内容。
