...
- **2026-09-22 已删 `DynamicObstacle` 及整个动态阻挡机制**（用户拍板：dynamicObstacles 已不重要）：删除 `Logic/DynamicObstacle.cs`（90 行）及 `.meta`；`GridMap.cs` 同步移除 `dynamicObstacles` 字段、`AddDynamicObstacle`/`RemoveDynamicObstacle`/`ClearDynamicObstacles`/`RefreshDynamicObstacles` 四个方法、`IsWalkable` 里的动态阻挡检查、Scene 视图 Gizmos 的紫色动态阻挡绘制。`GridCellType` 按用户要求保留为独立文件。README 目录树（Logic 4→3）与「逻辑层碰表现层」表（4 处→3 处）已同步。`dotnet build` 两个工程 0 错误通过。
- **2026-09-22 UI 代码归拢到 `Presentation/UI/`**（用户拍板）：`UIManager.cs` / `UIWindow.cs` / `SceneFadeUI.cs` / `SubtitleWindow.cs` 四个 uGUI 文件 git mv 至 `Presentation/UI/`（namespace 不变、无 asmdef 变化），README 目录树与「模块约定」第 1 条已同步（UI/ 成为四层目录下唯一允许的子目录）。两个 `.csproj` 的 Compile 路径已更新，`dotnet build` 0 错误通过。
- **2026-09-22 已删 `GridMap` 整链**（用户拍板：GridMap 还不能删除吗——验证后确认可以）：删除 `Presentation/GridMap.cs`（~440 行，含 `GridMapData`）及 `.meta`。删除前验证：全部 public API（LoadData/ScreenToPlane/FindPath/IsWalkable/CellGrid/WorldToGrid/GridToWorld）零调用方；脚本 GUID `df8f5f2c…` 在场景/预制体/材质中 0 引用；`FogOfWar` 消费 `map.cells` 不经过 GridMap。连带清理：`SceneObjectView.cs:162` 注释里的 GridMap 提及、README 目录树（Presentation 8→7）与「逻辑层碰表现层」引言、战争雾章节的 ⚠️ 注记（改为已删说明）。审查报告高严重度第 2 条关闭。`dotnet build` 两个工程 0 错误通过。
- **2026-09-22 已删旧世界残留**（用户拍板：可以删除了）：删除 `SmartVideoPlayer.cs`（475 行）+ `VideoFade.shader`、`BirdWanderer.cs`（装饰物，零引用）、`Materials/` 整个目录（6 个孤儿材质）、`Resources/RealMap.prefab`（空壳占位）、`Assets/Readme.asset`（URP 模板欢迎页）、11 个零引用 shader（BurningRoom 遗留 5 个、旧雾实现 5 个、`SelectionRing`、`BoxComposite`）。保留：`FogBlur`/`TextureRenderer`（在用的 2 个）与 `MaskEraseStamp`（清理文档 D5 拍板保留）。删除前逐项做了 GUID 反查（场景/预制体/材质 0 引用）与代码 grep 双验证。连带清理：csproj 两条 Compile 条目、README 目录树（Presentation 7→5、Resources 只剩 Shaders）、视频章节与「已停用未删」清单。审查报告高严重度第 1 条与低严重度第 9–11 条关闭。`dotnet build` 两个工程 0 错误通过。
