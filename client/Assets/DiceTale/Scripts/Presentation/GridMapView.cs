...
{
    /// <summary>
    /// **`GridMap` 协议组件在表现层的座位**：带网格对象 GameObject 上那个收 <see cref="MirrorMap"/>
    ...
    /// v16 起贴图不再在 `GridMap` 里（带网格的贴图的图在对象自己的 `ImageLayer` 里），
    /// 战争雾也不再从这里取格子（自己按 `mapId` 引用，数据由 <see cref="SceneObjectView"/>
    /// 解析后直接递给 <see cref="FogOfWar"/>），以后要画的**网格线**落在这里，
    /// 不会出现「第三处又抓了一份 map 数据」的第二真相。
    ///
    /// 数据本身仍归镜像（<see cref="MirrorObject.map"/>）：这里不拷格子、不解析，
    /// 只是把「这份数据是**现在生效的**」这个事实挂在对象自己身上——组件在，数据在；
    /// 组件被摘（对象不再是网格地图），数据自然也没了。
    /// </summary>
    ...
    public class GridMapView : MonoBehaviour
    {
        ...
        /// 所以这里收到的要么是合法数据、要么是「组件刚被摘掉」的 `null`
        /// （后者让以后画网格线的代码知道格子没了）。
        /// </summary>
